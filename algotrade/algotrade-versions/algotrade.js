	...
	analyze: function(product_id) {
		...
		if(_data.length > config.algorithm.minDataPoints) {
			...
			for(var c=_data.length - config.algorithm.minDataPoints;c < _data.length-1; c++) {	
				if(_data[c-1].price > _data[c].price + threshood && mover.trend > - 2  ) {
					mover.trend --;
				}
				else if(_data[c-1].price < _data[c].price - threshood && mover.trend < 2) {
					mover.trend ++;
				}
			}
			...
			for(var c = trend.length - 3; c < trend.length; c++) {			
